...
//
//-PURPOSE---------------------------------------------------------------------
// ProgramMgr class definition file.
//-----------------------------------------------------------------------------
//
//
//-CHANGE LOG------------------------------------------------------------------
...
public class ProgramMgr extends IcofUser {
  ...
  //-----------------------------------------------------------------------------
  public ProgramMgr(AppContext anAppContext
  ...
  //-----------------------------------------------------------------------------
  // Lookup a PM in the database, using the afs userid.
  //-----------------------------------------------------------------------------
  public void dbLookUpByName(AppContext anAppContext) throws IcofException {
    ...
    if (!dba.selectRow(anAppContext, getIntranetUserid(anAppContext))) {
      if (!dba.selectMarkedRow(anAppContext, getIntranetUserid(anAppContext))) {
        String msg = new String("Could not find program mgr name in PgmMgr table");
  ...
  //-----------------------------------------------------------------------------
  // Lookup a PM in the database, using the ID.
  //-----------------------------------------------------------------------------
  public void dbLookUpById(AppContext anAppContext) throws IcofException {
    ...
    if (!dba.selectRow(anAppContext, getID(anAppContext))) {
      if (!dba.selectMarkedRow(anAppContext, getID(anAppContext))) {
        String msg = new String("Could not find program mgr ID in PgmMgr table");
        IcofException ie = new IcofException(this.getClass().getName()
                                             ,getFuncName(anAppContext)
                                             ,IcofException.SEVERE
                                             ,msg
  ...
  //-----------------------------------------------------------------------------
  // Mark a PM as deleted in the database
  //-----------------------------------------------------------------------------
  public void dbMark(AppContext anAppContext) throws IcofException {
    ...
    // Now, mark this PM in the IcofUserRole table.
    Role thisRole = new Role(anAppContext
                             ,Constants.PROGRAM_MANAGER);
    IcofUserRole thisUserRole =
        new IcofUserRole(anAppContext
  ...
  //-----------------------------------------------------------------------------
  // Unmark a PM as deleted in the database
  //-----------------------------------------------------------------------------
  public void dbUnmark(AppContext anAppContext) throws IcofException {
    ...
    // Now, unmark this PM in the IcofUserRole table.
    Role thisRole = new Role(anAppContext
                             ,Constants.PROGRAM_MANAGER);
    IcofUserRole thisUserRole =
        new IcofUserRole(anAppContext
  ...
  //----------------------------------------------------------------------------
  // Add a PM to the database -- this function is PRIVATE because all
  ...
  //-----------------------------------------------------------------------------
  // Determine whether to add this program manager or to reinstate it
  //  (if it already exists, but has been marked as deleted)
  // Note -- there is no "mark as deleted" capability for program
  ...
  public String addOrReinstate(AppContext anAppContext) throws IcofException {
    ...
    // Finally, make sure this person is in the Program Mgr table.
    PgmMgrDBInterface dba =
        new PgmMgrDBInterface(anAppContext);
    if (dba.selectRow(anAppContext, getID(anAppContext))) {
      populate(anAppContext, dba);
    ...
    //
    // If we get here, it means this program manager was not in the
  ...
  //-----------------------------------------------------------------------------
  // Determine whether or not a ProgramMgr can be physically deleted from
  //   the database.  Also, return the list of active technologies associated with
  //   the ProgramMgr.
  //-----------------------------------------------------------------------------
  public String testDeletability(AppContext anAppContext
    ...
    //
    // A ProgramMgr is deletable only if:
    //   -- it is not associated with any technologies
    //
    // If associated only with "marked as deleted" technologies,
    //   the ProgramMgr can be "marked as deleted".
    //

    // Get the list of technologies associated with this ProgramMgr.
